...
public class EducationController {
   ...
   public String getAcademyList(Model m, 
    ...
    //뒤로가기 여기까지
     ...
     Page<EducationVO> elist = null;
      if(order.equals("")) {
    	  elist = eduRepo.AllSearchAndPagingQuery(paging, keywords);
    	  
      } else if(order.equals("star")) {
    	  Page<Object[]> temp = reviewRepository.avgStarDESC(paging, order);
    	  for(Object[] temp2 : temp) {
    		  EducationVO vo = new EducationVO();
    		  vo.setEdId((Integer)temp2[0]);
    		  vo.setEdTitle((String)temp2[1]);
    		  vo.setEdName((String)temp2[2]);
    		  vo.setEdKeyword((String)temp2[3]);
    		  vo.setEd_price((Integer)temp2[4]);
    		  vo.setAvg(Integer.parseInt(String.valueOf(temp2[5])));
    		  System.out.println(vo);
    		  elist.getContent().add(vo);
    		  System.out.println(elist.getContent());
    	  }
    	  // for(int i=0; i<temp.length)
    	  //elist = reviewRepository.avgStarDESC(paging, order);
    	  
    	  //elist = temp의 Object[] 값들을 EducationVO에 각각에 넣어서 elist를 만듬
    	  
    	  //  Page<EducationVO> elist
   ...
   public String getBoard(EducationVO vo, Model model,
      ...
      //상세페이지 리뷰정보용 + 페이징
      Page<ReviewVO> reviewList = reviewRepository.getReviewAndPaging(paging, edId);
